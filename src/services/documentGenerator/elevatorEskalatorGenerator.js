...
// Helper function untuk mengubah boolean menjadi centang atau kosong
...
// Helper function untuk mengubah boolean menjadi centang atau kosong (logika terbalik)
 ...
 * @param {object} data - Objek data audit dari Firestore.
...
const createLaporanElevator = async (data) => {
    ...
    const doc = new Docxtemplater(zip, {
        paragraphLoop: true,
        linebreaks: true,
        nullGetter: () => "" // Penting agar field kosong tidak error
    });
    
    const i = data.inspectionAndTesting; // Alias untuk mempermudah akses
    ...
    const renderData = {
        ...
        ventilationCarresult: i?.car?.ventilation?.result, // renamed to avoid conflict
        ...
        backupPowerARD2result: i?.electricalInstallation?.backupPowerARD?.result, // renamed to avoid conflict
        ...
        labelFireswitchresult: i?.electricalInstallation?.fireServiceElevator?.label?.result, // renamed
        ...
        operatingPanel2result: i?.electricalInstallation?.accessibilityElevator?.operatingPanel?.result, // renamed
        ...
        labelAccessibilityresult: i?.electricalInstallation?.accessibilityElevator?.label?.result, // renamed
    ...
    const fileName = `Laporan-Elevator-${data.generalData?.ownerName?.replace(/\s+/g, '-')}-${data.id}.docx`;
